...
TEST_F(NodeTest, rename_a_folder) {
    home->rename("HOME");
    std::list<string> result = home->findByName("my_profile");
    ASSERT_EQ("structure/HOME/my_profile", *(result.begin()));
    std::list<string> result2 = home->findByName("Documents");
    ASSERT_EQ("structure/HOME/Documents", *(result2.begin()));
}
