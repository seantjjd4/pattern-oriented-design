...
TEST_F(IteratorTest, OrderByNameIterator) {
    IteratorFactory * obnItFactory = new OrderByNameIteratorFactory();
    Iterator * obnIt = home->createIterator(obnItFactory);
...
TEST_F(IteratorTest, OrderByNameWithFolderFirstIterator) {
    IteratorFactory * obnwffItFactory = new OrderByNameWithFolderFirstIteratorFactory();
    Iterator * obnwffIt = home->createIterator(obnwffItFactory);

    obnwffIt->first();
    ASSERT_EQ("Documents", obnwffIt->currentItem()->name());

    obnwffIt->next();
    ASSERT_EQ("Downloads", obnwffIt->currentItem()->name());

    obnwffIt->next();
    ASSERT_EQ("my_profile", obnwffIt->currentItem()->name());
...
TEST_F(IteratorTest, OrderByKindIterator) {
    IteratorFactory * obkItFactory = new OrderByKindIteratorFactory();
    Iterator * obkIt = home->createIterator(obkItFactory);
